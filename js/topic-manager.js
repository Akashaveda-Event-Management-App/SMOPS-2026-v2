...
class TopicManager {
  ...
  generateTopicCard(topic, index) {
    ...
    return `
            <div class="topic-card glassmorphic-bg p-6 rounded-3xl border border-white/20 hover:border-${topic.color}-400/60 cursor-pointer group hover:shadow-2xl hover:shadow-${topic.color}-500/20 animate-fade-in-up h-full flex flex-col relative overflow-hidden" style="animation-delay: ${delay}s" onclick="openTopicModal('${topic.id}')">
                
                <!-- Decorative background glow -->
                <div class="absolute top-0 right-0 w-32 h-32 bg-${topic.color}-500/10 rounded-full blur-2xl -mr-16 -mt-16 transition-all duration-500 group-hover:bg-${topic.color}-500/20"></div>

                <!-- Header: Topic Number & Icon -->
                <div class="flex flex-col items-center flex-shrink-0 mb-4 relative z-10 w-full pt-2">
                    <!-- Topic Number (At Top) -->
                    <div class="text-xs lg:text-sm font-bold text-${topic.color}-400 uppercase tracking-widest mb-4 group-hover:text-${topic.color}-300 transition-colors duration-300">Topic ${topic.number}</div>
                    
                    <!-- Icon -->
                    <div class="w-16 h-16 lg:w-20 lg:h-20 bg-gradient-to-br from-${topic.gradientFrom} via-${topic.gradientVia} to-${topic.gradientTo} rounded-2xl flex items-center justify-center transition-all duration-300 shadow-lg shadow-${topic.color}-500/30 group-hover:shadow-${topic.color}-400/50 mb-4 group-hover:scale-110 transform">
                    ...
                    <!-- Separator -->
                    <div class="w-12 h-1 bg-gradient-to-r from-${topic.color}-400 to-${topic.gradientTo} rounded-full group-hover:w-20 transition-all duration-300"></div>
                </div>
                
                <!-- Title (Centered & Flexible) -->
                <div class="flex-grow flex items-center justify-center text-center mb-6 px-4 relative z-10 min-h-[5rem]">
                    <h3 class="text-lg lg:text-2xl font-bold text-white group-hover:text-${topic.color}-300 transition-colors duration-300 leading-snug line-clamp-3">
                        ${topic.title}
                    </h3>
                </div>
                
                <!-- Subtopics Counter (Pinned to Bottom) -->
                <div class="flex-shrink-0 w-full mt-auto relative z-10">
                    <div class="bg-gradient-to-r from-${topic.color}-500/10 to-${topic.gradientTo}/10 rounded-xl p-3 lg:p-4 border border-${topic.color}-400/20 group-hover:border-${topic.color}-400/40 transition-all duration-300 hover:bg-${topic.color}-500/20">
                        <div class="flex items-center justify-between gap-3">
                            <div class="text-base lg:text-lg font-semibold text-${topic.color}-200 group-hover:text-${topic.color}-100 transition-colors duration-300">${topic.subtopics.length} Subtopics</div>
                            <div class="w-8 h-8 bg-${topic.color}-400/20 rounded-full flex items-center justify-center group-hover:bg-${topic.color}-400/30 group-hover:translate-x-1 transform transition-all duration-300">
                                <svg class="w-4 h-4 text-${topic.color}-300 group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
